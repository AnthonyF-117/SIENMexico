                    ...
                    t &&
                        t.forEach(function (t, e) {
                            i.items.push(t.getAttribute(n.urlAttribute)),
                                ...
                                i.desc.push(t.getAttribute(n.descAttribute))
                                n.bindToItems &&
                                    i.addEvent(
                                        t,
                                        "click",
            ...
            prepareItem: function (t, e) {
                ...
                if ((this.loading(!0), this.options.videoRegex.test(i))) e.call(n, o('<div class="slbIframeCont"><iframe class="slbIframe" frameborder="0" allowfullscreen src="' + i + '"></iframe></div>'));
                    ...
                    (this.$currentImage = s.querySelector(".slbImage")),
                        this.options.showCaptions && this.captions[t] && s.appendChild(o('<div class="slbCaption-up">' + this.captions[t] + "</div>")),
                        this.loadImage(i, function () {
                            n.setImageDimensions(), e.call(n, s), n.loadImage(n.items[n.normalizePosition(n.currentPosition + 1)]);
                        });
                        switch(this.desc[t]){
                            case "Redes&Subestaciones":
                                var h = s.appendChild(o('<ul class="slbCaption-ul"></ul>'));
                                for(var i in Subestaciones){
                                    h.appendChild(o('<li>'+Subestaciones[i]+'</li>'))
                                }
                                //s.appendChild(o('<div class="slbCaption-up">asda</div>'))
                                break;
                            case "PanelesSolares":
                                var h = s.appendChild(o('<ul class="slbCaption-ul"></ul>'));
                                for(var i in PanelesSolares){
                                    h.appendChild(o('<li>'+PanelesSolares[i]+'</li>'))
                                }
                                break;
                            case "CalculoInstalacion":
                                console.log(CalculoInstalacion);
                                var h = s.appendChild(o('<ul class="littlebottom slbCaption-ul "></ul>'));
                                for(var i in CalculoInstalacion){
                                    if(CalculoInstalacion[i] == CalculoInstalacion.A){
                                        var u = h.appendChild(o('<li">'+CalculoInstalacion[i]+'</li>'));
                                        var l = u.appendChild(o('<ul class="littlefontsize"></ul>'));
                                        for(var k in A_VentaEquipos){
                                            l.appendChild(o('<li>'+A_VentaEquipos[k]+'</li>'));
                                        }
                                    }
                                    else{
                                        h.appendChild(o('<li>'+CalculoInstalacion[i]+'</li>'))
                                    }
                                }
                                break;
                        }
